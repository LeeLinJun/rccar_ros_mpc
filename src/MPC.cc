 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      fg[0] += 100 *CppAD::pow(vars[x_start + t]-tgx[t], 2) * CppAD::pow(0.9, t);
    ...
    for (int t = 1; t < N; t++) {
      if(t >= tgx.size()){
        ...
        fg[1 + x_start + t] = x1 - (x0 + ref_v/*v0*/ * CppAD::cos(psi0) * dt);
        fg[1 + y_start + t] = y1 - (y0 + ref_v/*v0 */* CppAD::sin(psi0) * dt);
        fg[1 + psi_start + t] = psi1 - (psi0 + /*v0*/ ref_v * delta0 / Lf * dt);
...
vector<double> MPC::Solve(std::vector<double> state, std::vector<double> ptsx, std::vector<double> ptsy) {
  ...
  //   vars_lowerbound[i] =  ref_v; 
  //   vars_upperbound[i] =  ref_v; 
  // }

  for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] =  -0.34; 
    vars_upperbound[i] =  0.34; 
  }
